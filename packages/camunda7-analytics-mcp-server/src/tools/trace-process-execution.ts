...
export function registerTraceProcessExecution(server: McpServer, ch: ClickHouseClient): void {
  server.tool(
    ...
    'Combine OTEL traces with process history for end-to-end execution visibility of a specific process instance.',
    {
      ...
      includeOtelSpans: z.boolean().default(true).describe('Include OTEL trace spans (requires otel database)'),
