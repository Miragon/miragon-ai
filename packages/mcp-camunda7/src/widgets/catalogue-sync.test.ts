...
/**
 * Guard against widget-catalogue drift: every widget registered in the host
 * bundle map (`widgets/index.ts`, i.e. `widgets/registry.ts` + the cockpit
 * app) must be catalogued in `definition.ts` and vice versa. A widget missing
 * from `definition.widgets` is invisible to `render-view`/`get-builder-catalogue`;
...
describe("widget catalogue ↔ registry sync", () => {
  it("definition.widgets matches the registered widget components exactly", () => {
    const registryIds = Object.keys(camunda7Widgets).sort()
