...
Shared [Konsist](https://github.com/LemonAppDev/konsist) base classes that encode the architectural conventions of the `plugins/` subprojects as JUnit tests. Rules live here once so every subproject can pick them up with a two-line `@Nested` wrapper.
