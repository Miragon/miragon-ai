...
/**
 * `MCP_OAUTH` — JSON config that turns the gateway into an OAuth resource
 ...
 * discovery metadata). The gateway never mints tokens.
 *
 ...
 * gateway's own `/authorize` + `/token` + `/register` using a pre-registered
 ...
 * forwarded to the engine per call, giving end-to-end user identity: gateway
...
const oidcSchema = z.object({
  ...
  // Back the gateway's local /authorize + /token compatibility routes
  ...
  // so the gateway needs outbound reachability to the issuer. Copy both
...
// enforced at the gateway before mcp-use sees the request
...
export interface McpOAuthSetup {
  ...
  /**
   * For `oidc-proxy` only: the redirect_uris the gateway's `/authorize` accepts.
...
function resolveClientSecret(config: {
  ...
  // A named-but-missing secret must fail the boot — the gateway would
 ...
 * handler (verified: a gateway `use()` middleware intercepts `/authorize`
 ...
 * `clientSecretEnvVar`) — consumed by the gateway, so the unknown-variable
