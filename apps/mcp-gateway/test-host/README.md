# Widget host simulation (`pnpm --filter @miragon-ai/mcp-gateway test:host`)
...
```bash
pnpm --filter @miragon-ai/mcp-gateway exec playwright install chromium   # once
pnpm --filter @miragon-ai/mcp-gateway test:host                          # builds the UI, then tests
```
